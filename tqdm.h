...
class tqdm {
    private:
		...
		b = static_cast<int>(v);
            ...
            int p = (int)(255.0f * (v * (1.f - s)));
            int q = (int)(255.0f * (v * (1.f - s * f)));
            int t = (int)(255.0f * (v * (1.f - s*(1.f - f))));
            v *= 255;
            i %= 6;
            int vi = (int)v;
            if (i == 0)      { r = vi; g = t;  b = p;  }
    ...
    public:
        ...
        void progress(int curr, int tot) {
            if(is_tty && (curr%period == 0)) {
                ...
                if (use_colors) {
                    if (color_transition) {
                        ...
                        hsv_to_rgb(static_cast<float>(0.0 + 0.01 * pct/3),
				   0.65f, 1.0f, r, g, b);
		  ...
		  printf("%s", bars[static_cast<size_t>(8.0*(fills-ifills))]);
